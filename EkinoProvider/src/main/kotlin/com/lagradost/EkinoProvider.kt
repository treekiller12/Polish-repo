...
open class ZaluknijProvider : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val lists = document.select("body > div.mainWrap > div.col-md-12.movie-wrap")
        val movies = lists[1].select("div:not(h4)")
        val series = lists[3].select("div:not(h4)")
        if (movies.isEmpty() && series.isEmpty()) return ArrayList()
        fun getVideos(type: TvType, items: Elements): List<SearchResponse> {
            return items.mapNotNull { i ->
                val href = i.selectFirst("a")?.attr("href") ?: return@mapNotNull null
                val img = div.cover-list > a > img
                    i.selectFirst("div.cover-list > a > img[src]")?.attr("src")?.replace("/thumb/", "/big/")
